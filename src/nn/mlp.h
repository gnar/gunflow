...
struct Layer {
  ...
  Layer(const int num_inputs, const int num_outputs,
        const TransferFunction transfer_fn)
      : num_inputs(num_inputs), num_outputs(num_outputs),
        transfer_fn(transfer_fn), W({num_inputs, num_outputs}),
        b({num_outputs}), x({num_inputs}), y({num_outputs}) {
...
public:
  MultilayerPerceptron(const int num_inputs, const int num_outputs,
    ...
    if (num_layers == 1) {
      ...
      layers.emplace_back(
          new Layer(num_inputs, layer_sizes[0], transfer_fns[0]));
      for (int l = 1; l < num_layers - 1; ++l) {
        layers.emplace_back(
            new Layer(layer_sizes[l - 1], layer_sizes[l], transfer_fns[l]));
      }
      layers.emplace_back(new Layer(layer_sizes[num_layers - 1], num_outputs,
                                    transfer_fns[num_layers - 1]));
    }
  }
